...
const normalizeIdentity = (value: unknown) => cleanValue(value).toUpperCase();
...
export const groupRowsForPDFVersion5 = (rows: ProductRow[]) => {
  const groups = new Map<string, GroupedProduct>();

  rows.filter(isProductRow).forEach((row, index) => {
    const itemNo = cleanValue(row.data?.ITEMNO);
    const normalizedItemNo = normalizeIdentity(itemNo);
    const barcode = cleanValue(row.barcode);
    const normalizedBarcode = normalizeIdentity(barcode);
    const identity = normalizedItemNo
    ...
    if (!existingGroup.imageSource && imageSource) {
      existingGroup.imageSource = imageSource;
    }
  });
...
const fitLabelLines = (
  ...
  let lastLine = fittedLines[maxLines - 1];
  while (lastLine && pdf.getTextWidth(`${lastLine}…`) > maxWidth) {
    lastLine = lastLine.slice(0, -1);
  }
  fittedLines[maxLines - 1] = lastLine ? `${lastLine}…` : "";
  return fittedLines;
};

export const generatePDFVersion5 = async (
  rows: ProductRow[],
  companyName: string,
  settings: Version5Settings,
  onProgress?: PDFProgressCallback
) => {
  const { default: jsPDF } = await import("jspdf");
  const pdf = new jsPDF("p", "mm", "a4");
  const products = groupRowsForPDFVersion5(rows);
  ...
  for (let pageIndex = 0; pageIndex < totalPages; pageIndex += 1) {
    ...
    for (let index = 0; index < pageProducts.length; index += 1) {
      ...
      const label = `${product.displayItemNo} (${product.quantity})`;
      const fontSize = Math.min(8, Math.max(5, cardWidth * 0.18));
      const lineHeight = fontSize * 0.36;
      const maxLines = Math.max(1, Math.min(2, Math.floor((labelHeight - 1) / lineHeight)));
      pdf.setTextColor(35, 35, 35);
      pdf.setFont("helvetica", "bold");
      pdf.setFontSize(fontSize);
      const labelLines = fitLabelLines(pdf, label, cardWidth - padding * 2, maxLines);
      const textBlockHeight = labelLines.length * lineHeight;
      const textY = labelY + (labelHeight - textBlockHeight) / 2 + lineHeight * 0.78;
      pdf.text(labelLines, x + cardWidth / 2, textY, {
        align: "center",
        lineHeightFactor: 1,
      });
  ...
  onProgress?.(100);
  pdf.save(`${filenamePrefix}Gopalam_Catalogue_Version_5_${fileDate(generatedAt)}.pdf`);
};
